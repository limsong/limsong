...
if (trim($G_INJECTION_PATTERN_CHK) == "") {
        $G_INJECTION_PATTERN_CHK = "";         //<-- 수정가능부분 : 기본값 Y, Y - injection 패턴 값으로 체크함,    N - 프로그램을 실행안함
        //    코드 예) G_INJECTION_PATTERN_CHK = "N"
}

if (trim($G_INJECTOIN_MAIL_SEND) == "") {
        $G_INJECTOIN_MAIL_SEND = "N";           //<-- 수정가능부분 : 기본값 Y, Y - injectoin 패턴 값으로 체크될 경우 메일보냄,  N - 메일 안 보냄
        //    코드 예) G_INJECTOIN_MAIL_SEND = "N"
}

if (trim($G_INJECTOIN_MAIL_ADDR) == "") {
        $G_INJECTOIN_MAIL_ADDR = "";           //<-- 수정가능부분 : G_INJECTOIN_MAIL_SEND 값이 Y일 경우 G_INJECTOIN_MAIL_ADDR 정보로
...
if (trim($G_INJECTOIN_URL_CHANGE) == "") {
        $G_INJECTOIN_URL_CHANGE = "";          //<-- 수정가능부분 : 기본값 Y, Y - injectoin 패턴 값으로 체크될 경우
        //                   G_INJECTOIN_URL_NAME 값으로 페이지 이동, N - URL 이동안함
        //    코드 예) G_INJECTOIN_URL_CHANGE = "N"
}

if (trim($G_INJECTOIN_URL_NAME) == "") {
        $G_INJECTOIN_URL_NAME = "";                //<-- 수정가능부분 : 기본값 기본도메인, G_INJECTOIN_URL_CHANGE 값이 Y이고
...
if (trim($G_INJECTOIN_URL_ALERT) == "") {
        $G_INJECTOIN_URL_ALERT = "";            //<-- 수정가능부분 : 기본값 "유효하지 않은 문자가 체크 되었습니다."
...
If ($G_INJECTION_PATTERN_CHK == "Y") {
        ...
        if (trim($G_INJECTOIN_URL_NAME) == "") {
                ...
                $G_INJECTOIN_URL_NAME = "http://sozo.bestvpn.net/";
        }
        ...
        foreach ($_POST as $key => $post_value) {
                if (eregi($G_INJECTION_PATTERN, $post_value)) {
        ...
        if ($G_INJECTION_CHECK_VALUE == False) {
                foreach ($_GET as $key => $get_value) {
                        if (eregi($G_INJECTION_PATTERN, $get_value)) {
        ...
        if ($G_INJECTION_CHECK_VALUE == False) {
                ...
                if (eregi($G_INJECTION_PATTERN, $req_value)) {
