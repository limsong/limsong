...
$G_INJECTION_PATTERN_CHK = "";    //기본값 Y, Y - injection 패턴 값으로 체크함,    N - 프로그램을 실행안함

$G_INJECTOIN_MAIL_SEND   = "";    //기본값 Y, Y - injectoin 패턴 값으로 체크될 경우 메일보냄,  N - 메일 안 보냄
...
$G_INJECTOIN_URL_CHANGE  = "";   //기본값 Y, Y - injectoin 패턴 값으로 체크될 경우 $G_INJECTOIN_URL_NAME 값으로 페이지 이동,  N - URL이동안함

$G_INJECTOIN_URL_NAME    = "";   //기본값 기본도메인, $G_INJECTOIN_URL_CHANGE 값이 Y이고 injectoin 패턴 값으로 체크되는 경우 이동하는 URL값
//코드 예) http://www.gabia.com <==  http://부터 모두 작성해주셔야 합니다.

$G_INJECTOIN_URL_ALERT   = "";   //'기본값 "유효하지 않은 문자가 체크 되었습니다.", $G_INJECTOIN_URL_NAME로 이동하기 전 나오는 메시지
