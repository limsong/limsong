...
$number=$_GET['number'];
$query="select comment,fid,thread from $code where uid='$number'";
$result=mysql_query($query) or die($query);
$ou_comment=stripslashes(mysql_result($result,0,0));
$ou_fid=mysql_result($result,0,1);
$ou_thread=mysql_result($result,0,2);
?>
...
<div id="total">
	...
	<div id="main">
		...
		<dl id="readContent">
		<form name="bForm" action="boardRePost.php?code=<?=$code?>&thread=<?=$ou_thread?>&fid=<?=$ou_fid?>"
		target="action_frame" onsubmit="return checkBForm(this)" method="post" enctype="multipart/form-data">
		<table cellpadding="5" border="0" cellspacing="1" width="610" height="20">
				...
				<tr>
					<td>비밀번호</td>
					<td><input class="inp" name="passwd" type="password" size="18" /></td>
				</tr>
				<tr>
					<td>파일1</td>
