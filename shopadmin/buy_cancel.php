...
<div class="pageNavi" style="text-align: center;">
        ...
        if ($page > 1) {
                echo("<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=1'><input type=\"button\" class=\"memEleB\" value=\"처음으로가기\" /></a>");
        }
        if ($block > 1) {
                $bPage = $first_page - 1;
                echo "[<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=$bPage'>[이전 " . $bpage_per_block . "개]</a>] ";
        }
        if ($page > 1) {
                $bfPage = $page - 1;
                echo("<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=$bfPage'> <input type=\"button\" class=\"memEleB\" value=\"검색\" /></a>");
        }
        ...
        for ($my_page = $first_page; $my_page <= $last_page; $my_page++) {
                if ($page == $my_page) {
                        ...
                        echo(" [<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=$my_page'>" . $my_page . "</a>]");
                }
        }
        if ($page < $total_page) {
                $nxPage = $page + 1;
                echo("<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=$nxPage'><input type=\"button\" class=\"memEleB\" value=\"검색\" /></a>");
        }
        if ($block < $total_block) {
                $nPage = $last_page + 1;
                echo "[<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=$nPage'>[다음 " . $bpage_per_block . "개]</a>]";
        }
        if ($page < $total_page) {
                echo("<a href='orderList.php?code=claim&delivery=32_1&key=$key&keyfield=$keyfield&page=$total_page'> <input type=\"button\" class=\"memEleB\" value=\"검색\" /></a>");
        }
        ?>
</div>
<form name="searchForm" method="post" action="orderList.php?code=claim&delivery=32_1">
