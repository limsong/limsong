                ...
                <table align="center" width="100%" class="memberListTable" border="0" cellspacing="0" ellpadding="0">
                        ...
                        while ($row = mysql_fetch_assoc($result)) {
                                ...
                                if ($buy_claim_status == "128") {
                                        $str = '<input type="button" class="memEleB cancel" data="R" data-status="1" data-seq="' . $buy_claim_seq . '" data-num="128" value="환불신청">';
                                } elseif ($buy_claim_status == "256") {
                                        $str = '환불완료';
                                }

                                if ($buy_claim_status == "8192") {
                                        $str = '<input type="button" class="memEleB cancel" data="R" data-status="1" data-seq="' . $buy_claim_seq . '" data-num="8192" value="교환신청">';
                                } elseif ($buy_claim_status == "65536") {
                                        $str = '교환완료';
                                }
...
<div class="pageNavi" style="text-align: center;">
        ...
        if ($page > 1) {
                echo("<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=1'><input type=\"button\" class=\"memEleB\" value=\"처음으로가기\"/> </a>");
        }
        if ($block > 1) {
                $bPage = $first_page - 1;
                echo "[<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=$bPage'>[이전 " . $bpage_per_block . "개]</a>] ";
        }
        if ($page > 1) {
                $bfPage = $page - 1;
                echo("<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=$bfPage'> <input type=\"button\" class=\"memEleB\" value=\"이전페이지\"/></a>");
        }
        ...
        for ($my_page = $first_page; $my_page <= $last_page; $my_page++) {
                if ($page == $my_page) {
                        ...
                        echo(" [<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=$my_page'>" . $my_page . "</a>]");
                }
        }
        if ($page < $total_page) {
                $nxPage = $page + 1;
                echo("<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=$nxPage'> <input type=\"button\" class=\"memEleB\" value=\"다음페이지\"/></a>");
        }
        if ($block < $total_block) {
                $nPage = $last_page + 1;
                echo "[<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=$nPage'>[다음 " . $bpage_per_block . "개]</a>]";
        }
        if ($page < $total_page) {
                echo("<a href='orderList.php?code=claim&delivery=128_4&key=$key&keyfield=$keyfield&page=$total_page'> <input type=\"button\" class=\"memEleB\" value=\"검색\"/></a>");
        }
        ?>
</div>
<form name="searchForm" method="post" action="orderList.php?code=claim&delivery=128_4">
