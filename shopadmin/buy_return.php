...
<div id="maninfo">
        <form name="orderListForm" action="">
                <table align="center" width="100%" class="memberListTable" border="0" cellspacing="0" ellpadding="0">
                        ...
                        <?php
                        $currentTime = time();
                        $query = "SELECT * FROM buy_claim  $addQuery ORDER BY buy_seq DESC limit $first,$bnum_per_page";
                        ...
                        while ($row = mysql_fetch_assoc($result)) {
                                ...
                                if ($buy_claim_status == "512") {
                                        $str = '<input type="button" class="memEleB cancel" data="T" data-status="2" data-seq="' . $buy_claim_seq . '" data-num="512" value="반품접수">';
                                } elseif ($buy_claim_status == "1024") {
                                        $str = '<input type="button" class="memEleB cancel" data="T" data-status="2" data-seq="' . $buy_claim_seq . '" data-num="1024" value="반품배송중">';
                                } elseif ($buy_claim_status == "2048") {
                                        $str = '<input type="button" class="memEleB cancel" data="T" data-status="2" data-seq="' . $buy_claim_seq . '" data-num="2048" value="반품환불">';
                                } elseif ($buy_claim_status == "4096") {
                                        $str = '반품완료';
                                }
                                ...
                                while ($buy_claim_goods_row = mysql_fetch_array($buy_claim_goods_result)) {
                                        ...
                                        if ($i == 0) {
                                                ...
                                                <tr class="contentTr" onmouseover="this.style.backgroundColor='#f0f0f0'" onmouseout="this.style.backgroundColor=''">
                                                        <td align="center" rowspan="<?= $rowspan ?>" height="30">
...
<div class="pageNavi" style="text-align: center;">
        ...
        if ($page > 1) {
                echo("<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=1'><input type=\"button\" class=\"memEleB\" value=\"처음으로가기\"/></a>");
        }
        if ($block > 1) {
                $bPage = $first_page - 1;
                echo "[<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=$bPage'>[이전 " . $bpage_per_block . "개]</a>] ";
        }
        if ($page > 1) {
                $bfPage = $page - 1;
                echo("<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=$bfPage'> <input type=\"button\" class=\"memEleB\" value=\"이전페이지\"/></a>");
        }
        ...
        for ($my_page = $first_page; $my_page <= $last_page; $my_page++) {
                if ($page == $my_page) {
                        ...
                        echo(" [<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=$my_page'>" . $my_page . "</a>]");
                }
        }
        if ($page < $total_page) {
                $nxPage = $page + 1;
                echo("<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=$nxPage'><input type=\"button\" class=\"memEleB\" value=\"다음페이지\"/></a>");
        }
        if ($block < $total_block) {
                $nPage = $last_page + 1;
                echo "[<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=$nPage'>[다음 " . $bpage_per_block . "개]</a>]";
        }
        if ($page < $total_page) {
                echo("<a href='orderList.php?code=claim&delivery=512&key=$key&keyfield=$keyfield&page=$total_page'><input type=\"button\" class=\"memEleB\" value=\"마지막으로가기\"/></a>");
        }
        ?>
</div>
<form name="searchForm" method="post" action="orderList.php?code=claim&delivery=512">
