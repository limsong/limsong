<form name="boardListForm" method="post" action="boardListDelPost.php?code=<?= $code ?>&page=<?= $page ?>&key=<?= $key ?>&keyfiled=<?= $keyfiled ?>" target="action_frame">
    <input type="hidden" name="tbl" value="tbl_bbs">
    <table align="center" width="100%" class="memberListTable">
        ...
        $currentTime = time();
        $query = "select * from tbl_bbs $addQuery order by uid desc limit $first,$bnum_per_page";
        $result = mysql_query($query) or die($query);

        while ($row = mysql_fetch_assoc($result)) {                 //연관 배열
            $ou_uid = $row["uid"];
            $ou_comment = stripslashes($row["comment"]);        //역슬래시를 없애준다.두개면 하나만 없앤다.stripslashes();
            ...
            <tr class="contentTr">
                ...
                <td class="memId" style="padding:10px;">
                    <?= $reImg ?>
                    <a href="javascript:void(0);"
                       data="user_review=yes&code=<?= $code ?>&number=<?= $ou_uid ?>&page=<?= $page ?>&keyfield=<?= $keyfield ?>&key=<?= $key ?>"
                       class="ifDiv review-msg"><?=$ou_comment?>
                    </a>
                    <?= $newImg ?>
                </td>
