        ...
        <form name="orderListForm" method="post" action="buyDlvPost.php?page=<?= $page ?>&keyfield=<?= $keyfield ?>&key=<?= $key ?>" target="action_frame">
                ...
                <table align="center" width="100%" class="memberListTable" border="0" cellspacing="0" ellpadding="0">
                        ...
                        while ($row = mysql_fetch_assoc($result)) {
                                ...
                                $buy_dlv_name = $row["buy_dlv_name"];//주문인
...
<div class="pageNavi" style="text-align: center;">
        ...
        if ($page > 1) {
                echo("<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=1'> <input type=\"submit\" class=\"memEleB\" value=\"처음으로가기\"/></a>");
        }
        if ($block > 1) {
                $bPage = $first_page - 1;
                echo "[<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=$bPage'>[이전 " . $bpage_per_block . "개]</a>] ";
        }
        if ($page > 1) {
                $bfPage = $page - 1;
                echo("<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=$bfPage'><input type=\"submit\" class=\"memEleB\" value=\"이전페이지\"/></a>");
        }
        ...
        for ($my_page = $first_page; $my_page <= $last_page; $my_page++) {
                if ($page == $my_page) {
                        ...
                        echo(" [<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=$my_page'>" . $my_page . "</a>]");
                }
        }
        if ($page < $total_page) {
                $nxPage = $page + 1;
                echo("<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=$nxPage'> <input type=\"submit\" class=\"memEleB\" value=\"다음페이지\"/></a>");
        }
        if ($block < $total_block) {
                $nPage = $last_page + 1;
                echo "[<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=$nPage'>[다음 " . $bpage_per_block . "개]</a>]";
        }
        if ($page < $total_page) {
                echo("<a href='orderList.php?code=buy&delivery=8&key=$key&keyfield=$keyfield&page=$total_page'><input type=\"submit\" class=\"memEleB\" value=\"마지막으로가기\"/></a>");
        }
        ?>
</div>
<form name="searchForm" method="post" action="orderList.php?code=buy&delivery=8">
